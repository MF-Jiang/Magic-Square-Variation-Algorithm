...
# swap dia to create neighborhood
...
def tabu_search(magic_square, cost_func, max_iter, tabu_size, M):
    ...
    for i in range(max_iter):
        ...
        if (tuple(map(tuple, new_solution)) not in tabu_list) and (new_cost < current_cost):
        ...
        tabu_list.append(tuple(map(tuple, new_solution)))
...
def variable_neighborhood_search(matrix, ts_params, M):
    # first local search
    magic_square = tabu_search(np.copy(matrix), objective_function, *ts_params)
    ...
    while time.time() - start_time <= 98:
        ...
        while i <= (matrix.shape[0] + 5) ** 2:
            ...
            if swap >= 8:
                swap = 0
        # local search
        new_magic_square = tabu_search(magic_square, objective_function, *ts_params)
        new_cost = objective_function(new_magic_square, M)
...
def main():
    ...
    x_optimal = variable_neighborhood_search(x0, (5000, (n + 5) ** 2, M), M)
    print(M)
